...
void IRAM_ATTR audioSampleTimer()
{
	...
	// if (s != 0)
	// 	log_d("%d = %d", s, s >> (16 - HW_AUDIO_RESOLUTION_BITS));
	LEDC.channel_group[0].channel[2].duty.duty = s >> (16 - HW_AUDIO_RESOLUTION_BITS);
	...
	LEDC.conf.clk_en = 1; // register clock gating
	#else
